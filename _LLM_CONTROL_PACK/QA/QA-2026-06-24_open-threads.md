...
| T-03 | **pdfcadcore sync manifest** | Sync gate red: manifest hash stale vs post-R5 `import_report.py` | WS-SYNC | Needs FC owner manifest bump |
...
| # | Thread | Context | Asked by | Status |
|---|--------|---------|----------|--------|
| T-04 | **LC portable vs native plugin** | Both artifacts exist; field docs say portable-first; launcher discovery added — which is canonical for human confirmation? | Reviewer B | Open |
| T-05 | **Blender COMPATIBILITY.md vs 5.1** | Smoke proves cp310-abi3 wheel on Python 3.13; doc still warns cp311-only | Reviewer C | Open |
...
| T-08 | **R4-06 LC/BL `--preflight` one-liner** | Preflight copy in INSTALL; CLI flag not wired | WS-R5 | Deferred |

...
| **R5-4** | Omni-box + scan routing backlog | **P0:** Rational storage fix in `lib/core/omni/` (191 tests green but P0 precision defects remain per `QA-2026-07-03_app-omni-coordination.md`). **P1:** `OmniIntentKind.scan` + time/weight routing without mode picker; offline pending-sync badge on home/omni; time-parse error suggestions (`OmniErrorResponse.suggestion`). **P1a–P2 domain packs (Q-N9 / A-R→N9):** weight/CG → fab geometry → galv vent/drain (AGA cited); grade capacity **out of scope**. **P2:** compound inventory (location/bay field); tape step `@N` → time entry description. **Capture channel rank (Q-N8):** photo-as-record → voice-as-text → MTR OCR → count-by-camera out of scope v1. Each slice: one golden omni phrase test. | **OPEN** | Q-P3, Q-N8, Q-N9, A-P→N8, A-Q→P3, A-R→N9. `omni_intent.dart` lacks scan; inventory filter has no bay parser. |
...
4. **R5-4** — omni P0 rational fix; scan intent; sync badge; golden phrase tests.

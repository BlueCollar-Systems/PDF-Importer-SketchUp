...
2026-06-26 01:10 UTC | WS-RPIPE | LC token follow-up (383d5320) | DONE | Live verify: GitHub --latest LC v1.0.48; no v1.0.49 tag (404). Source main still 1.0.48; post-release commits are [skip release] CI only. Portable asset first on release page. Deleted repo secret RELEASE_BUMP_TOKEN. Website repo-metadata + index badge v1.0.48 aligned; no new release run required.

2026-06-26 18:40 UTC | WS-UXREG | Anonymous reviewer | INTAKE | Product-owner field report (3 screenshots), owner away until Sun/Mon. (1) SU popup "LibreCAD users: this is SketchUp" + every-import modal — CONFIRMED already removed from main.rb import_pdf by collective (open UX: relocate mode hints into import dialog / one-time notice). (2) Alignment/scale REGRESSION — reproduced via pdfcadcore.extract_page on 1017 p0 (431 text items). Root cause A (P0): inline stacked fractions merge to a FLAT full-size token (`1/8`) that overruns adjacent whole-number text in FreeCAD ShapeString placement (matches `4'-7/2`,`1'-0¾`,`13'-5/8`,`2 13/16⌀`); whole-number+fraction also stay split (`'1'-3'` + `'1/8'`). Root cause B (P1): SU rotated BOM-QUAN digits live in the SketchUp Ruby port — pdfcadcore extracts those at rotation=0 (correct); likely regression of v3.7.59 vertical-BOM fix; repro corpus-stress/01_bom_vertical_quantities.pdf. Full spec + repro + plan: QA-2026-06-26_user-report-popup-and-alignment-regression.md. NEXT: core regression test first, then fix + IN-HOST visual verify (FC + SU 2017) — field-test gate, can't be confirmed headless.

2026-06-26 14:30 UTC | WS-REGRESS | Popup/BOM/FC text regression | DONE | SU v3.7.74 show-once ImportGuidance; BOM column-aware QUAN + native rotated labels; FC v4.0.54 span bbox-fit + scaled raster. Desktop QA trio + mirror. Tests: SU label/mode/pre_import/ruby22 PASS; FC 19 text/report PASS.

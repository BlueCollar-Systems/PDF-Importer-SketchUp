    ...
    module PdfSalvage
      ...
      class << self
        ...
        def run_pdftocairo(exe, input, output)
          ...
          validation = PopplerResultValidator.validate(res,
            :executable => exe,
            :argv => args,
            :context => 'PdfSalvage',
            :attempt => 1,
          ...
          { :ok => false, :reason => :process_failed,
            :incomplete_output => false, :rejection_scope => :helper_attempt,
            :evidence => { :executable => exe.to_s, :stdout => '',
                           :stderr => e.message, :artifacts => [] } }
        end

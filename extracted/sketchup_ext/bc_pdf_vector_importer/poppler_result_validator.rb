...
# Central, fixture-driven validation for active Poppler helper boundaries.
...
module BlueCollarSystems
  module PDFVectorImporter
    module PopplerResultValidator
      ...
      # observed when the pinned Poppler build drops the Adobe-GB1 line from
      # the public cid_identity_h fixture while still returning exit status 0.
      ...
      def self.proven_adobe_gb1_fixture_incomplete_output?(stdout, stderr)
        # stdout and stderr are separate attempt-local streams.  Combining
        # fragments across them can synthesize a cluster no helper emitted.
      ...
      # Compatibility name for callers outside this repository.  Its behavior
      # remains deliberately limited to the proven Adobe-GB1 fixture cluster.
      ...
      def self.log_rejection(result, context = nil)
        ...
        Logger.warn(label.to_s, message) if defined?(Logger) && Logger.respond_to?(:warn)
      rescue StandardError
        # Logging must not turn a rejected helper attempt into a host crash.
        nil
      end
      ...
      def self.artifact_evidence(paths)
        Array(paths).compact.map do |path|
          begin
            ...
            { :path => value, :exists => false, :bytes => nil, :error => e.message }
          end
        end
      end
